...
def alert_overdue_projects(count: int) -> None:
        """
        Generate alerts based on the number of overdue projects.
        
        Args:
            count: The number of overdue projects detected in the audit.
        
        Returns:
            None
        
        Logs:
            INFO message with alert level based on thresholds:
            - 0-2: No alert
            - 3-5: Warning
            - 6+: Critical
        """
        if count <= 2:
            log.info(f"✅ All good! Only {count} overdue projects.")
        elif 3 <= count <= 5:
            log.warning(f"⚠️ Warning: {count} overdue projects detected!")
        else:
            log.error(f"❌ Critical Alert: {count} overdue projects! Immediate action required!")
